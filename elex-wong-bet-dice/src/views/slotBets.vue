...
export default class Bets extends Vue {
  ...
  mounted() {
    ...
    (window as any).$bus.on('SlotBet', this.setAllTransactionData);
  ...
  beforeUnmount() {
    ...
    (window as any).$bus.off('SlotBet', this.setAllTransactionData);
  }
  ...
  loadMore() {
    ...
    try {
      switch(this.listShowType) {
        case "my":
          this.slotProgram.getWalletTransactions(this.walletTransactionLast).then((data:any) => {
        ...
        case "win":
          this.slotProgram.getProgramTransactionWins(this.winTransactionPage).then((data:any) => {
            this.isLoadMore = false;
            if (data.length == 30) {
        ...
        case "rank":
          this.slotProgram.getProgramRanks().then((data:any) => {
            this.isLoadMore = false;
            this.rankList = data;
          }).catch(() => {
            this.isLoadMore = false;
        ...
        default:
          this.slotProgram.getProgramTransactions(this.allTransactionLast).then((data:any) => {
            this.isLoadMore = false;
