<template>
    <div class="crypto-exchange-list">
...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('Crypto/Exchanges/getList', this.searchField).then(result => {
            ...
            create() {
                this.$router.push({path: '/crypto/exchange/add'});
            },
            notify(id) {
            ...
            edit(id) {
                this.$router.push({path: '/crypto/exchange/' + id});
            },
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    this.isLoading = true;
                    this.$api.requestByPost('Crypto/Exchanges/delete', {id: id}).then(result => {
                        if(result.status == true) {
                            this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                                showOnlyTheLastOne: true,
                            });
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/crypto/exchangeList.scss";
    @import "/assets/scss/mobile/crypto/exchangeList.scss";
</style>
