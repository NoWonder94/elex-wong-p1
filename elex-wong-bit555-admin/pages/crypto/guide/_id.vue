<template>
    <div class="crypto-guide-detail">
        <div class="detail-title">
            {{ $t('menu.guide') + $t('page.detail') }}
        </div>
        <div v-if="isEdit">
        ...
        methods: {
            init() {
                if(this.id > 0) {
                    ...
                    this.$api.requestByPost('Crypto/Guides/getDetail', {id: this.id}).then(result => {
            ...
            create() {
                ...
                this.isLoading = true;
                this.$api.requestByPost('Crypto/Guides/create', form).then(result => {
                    if(result.status == true) {
                        this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                            showOnlyTheLastOne: true,
                        });
                        this.$router.push({path: '/crypto/guide/list'});
                    } else{
                        this.$notiflix.Notify.failure(result.msg ? result.msg : this.$t('message.error'), {
                            showOnlyTheLastOne: true,
                        });
            ...
            update() {
                ...
                this.isLoading = true;
                this.$api.requestByPost('Crypto/Guides/update', form).then(result => {
                    if(result.status == true) {
                        this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                            showOnlyTheLastOne: true,
                        });
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/crypto/guideDetail.scss";
    @import "/assets/scss/mobile/crypto/guideDetail.scss";
</style>
