...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('Crypto/Guides/getList', this.searchField).then(result => {
            ...
            create() {
                this.$router.push({path: '/crypto/guide/add'});
            },
            notify(id) {
            ...
            edit(id) {
                this.$router.push({path: '/crypto/guide/' + id});
            },
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    this.isLoading = true;
                    this.$api.requestByPost('Crypto/Guides/delete', {id: id}).then(result => {
                        if(result.status == true) {
                            this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                                showOnlyTheLastOne: true,
                            });
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/crypto/guideList.scss";
    @import "/assets/scss/mobile/crypto/guideList.scss";
</style>
