<template>
    <div class="crypto-market-cap-list">
...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('Crypto/MarketCap/getList', this.searchField).then(result => {
                    if(result.status) {
            ...
            create() {
                this.$router.push({path: '/crypto/marketCap/add'});
            },
            edit(id) {
                this.$router.push({path: '/crypto/marketCap/' + id});
            },
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    this.isLoading = true;
                    this.$api.requestByPost('Crypto/MarketCap/delete', {id: id}).then(result => {
                        if(result.status) {
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/crypto/marketCapList.scss";
    @import "/assets/scss/mobile/crypto/marketCapList.scss";
</style>
