...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('Crypto/News/getList', this.searchField).then(result => {
            ...
            create() {
                this.$router.push({path: '/crypto/news/add'});
            },
            notify(id) {
            ...
            edit(id) {
                this.$router.push({path: '/crypto/news/' + id});
            },
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    this.isLoading = true;
                    this.$api.requestByPost('Crypto/News/delete', {id: id}).then(result => {
                        if(result.status == true) {
                            this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                                showOnlyTheLastOne: true,
                            });
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/crypto/newsList.scss";
    @import "/assets/scss/mobile/crypto/newsList.scss";
</style>
