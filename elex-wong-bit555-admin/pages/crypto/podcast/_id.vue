<template>
    <div class="crypto-podcast-detail">
        <div class="detail-title">
            {{ $t('menu.podcast') + $t('page.detail') }}
        </div>
        <div v-if="isEdit">
        ...
        methods: {
            init() {
                if(this.id > 0) {
                    ...
                    this.$api.requestByPost('Crypto/Podcast/getDetail', {id: this.id}).then(result => {
                        if(result.status) {
            ...
            create() {
                ...
                this.isLoading = true;
                this.$api.requestByPost('Crypto/Podcast/create', form).then(result => {
                    if(result.status) {
                        this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                            showOnlyTheLastOne: true,
                        });
                        this.$router.push({path: '/crypto/podcast/list'});
                    } else{
                        this.$notiflix.Notify.failure(result.msg ? result.msg : this.$t('message.error'), {
                            showOnlyTheLastOne: true,
                        });
            ...
            update() {
                ...
                this.isLoading = true;
                this.$api.requestByPost('Crypto/Podcast/update', form).then(result => {
                    if(result.status) {
                        this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                            showOnlyTheLastOne: true,
                        });
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/crypto/podcastDetail.scss";
    @import "/assets/scss/mobile/crypto/podcastDetail.scss";
</style>
