...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('Crypto/Podcast/getList', this.searchField).then(result => {
                    if(result.status) {
            ...
            create() {
                this.$router.push({path: '/crypto/podcast/add'});
            },
            edit(id) {
                this.$router.push({path: '/crypto/podcast/' + id});
            },
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    this.isLoading = true;
                    this.$api.requestByPost('Crypto/Podcast/delete', {id: id}).then(result => {
                        if(result.status) {
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/crypto/podcastList.scss";
    @import "/assets/scss/mobile/crypto/podcastList.scss";
</style>
