<template>
    <div class="crypto-price-tracker-list">
...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('Crypto/PriceTracker/getList', this.searchField).then(result => {
            ...
            create() {
                this.$router.push({path: '/crypto/priceTracker/add'});
            },
            notify(id) {
                this.$confirm(this.$t('message.notify'), {
                    ...
                    let form = {
                        table_name: 'crypto_priceTracker',
                        table_id: id,
                    };
                    this.isLoading = true;
            ...
            edit(id) {
                this.$router.push({path: '/crypto/priceTracker/' + id});
            },
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    this.isLoading = true;
                    this.$api.requestByPost('Crypto/PriceTracker/delete', {id: id}).then(result => {
                        if(result.status == true) {
                            this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                                showOnlyTheLastOne: true,
                            });
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/crypto/priceTrackerList.scss";
    @import "/assets/scss/mobile/crypto/priceTrackerList.scss";
</style>
