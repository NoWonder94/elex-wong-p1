...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('Crypto/Videos/getList', this.searchField).then(result => {
                    if(result.status) {
            ...
            create() {
                this.$router.push({path: '/crypto/video/add'});
            },
            edit(id) {
                this.$router.push({path: '/crypto/video/' + id});
            },
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    this.isLoading = true;
                    this.$api.requestByPost('Crypto/Videos/delete', {id: id}).then(result => {
                        if(result.status) {
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/crypto/videoList.scss";
    @import "/assets/scss/mobile/crypto/videoList.scss";
</style>
