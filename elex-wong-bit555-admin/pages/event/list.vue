...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('Event/getList', this.searchField).then(result => {
            ...
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    //confirm
                    this.isLoading = true;
                    this.$api.requestByPost('Event/delete', {id: id}).then(result => {
                        if(result.status == true) {
                            this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                                showOnlyTheLastOne: true,
                            });
...
<style lang="scss" type="text/scss">
    @import "/assets/scss/pc/eventList.scss";
    @import "/assets/scss/mobile/eventList.scss";
</style>
