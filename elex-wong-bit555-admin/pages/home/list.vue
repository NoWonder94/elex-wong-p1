...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('Home/getList', this.searchField).then(result => {
            ...
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    // confirm
                    this.isLoading = true;
                    this.$api.requestByPost('Home/delete', {id: id}).then(result => {
                        if(result.status == true) {
                            this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                                showOnlyTheLastOne: true,
                            });
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/homeList.scss";
    @import "/assets/scss/mobile/homeList.scss";
</style>
