...
<script>
    export default {
        ...
        methods: {
            init() {
                this.isLoading = true;
                this.$api.requestByPost('News/getList', this.searchField).then(result => {
            ...
            remove(id) {
                this.$confirm(this.$t('message.delete') + id + '?', {
                    ...
                    this.isLoading = true;
                    this.$api.requestByPost('News/delete', {id: id}).then(result => {
                        if(result.status == true) {
                            this.$notiflix.Notify.success(result.msg ? result.msg : this.$t('message.success'), {
                                showOnlyTheLastOne: true,
                            });
...
<style lang="scss" type="text/css">
    @import "/assets/scss/pc/newsList.scss";
    @import "/assets/scss/mobile/newsList.scss";
</style>
