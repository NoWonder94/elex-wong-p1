...
use App\Application\Api\Models\Crypto\Home;
use Config, Lang;

class HomeService extends \App\Services\Crypto\HomeService {
	public function getList($page = DEFAULT_PAGE, $pageSize = DEFAULT_PAGE_SIZE) {
		$isTopPin               = Home::where('status', '=', 1)
                                      ->where('is_top', '=', 2)
                                      ->get()
                                      ->toArray();

        $isMiddlePin            = Home::where('status', '=', 1)
                                      ->where('is_top', '=', 1)
                                      ->orderBy('update_time', 'desc')
                                      ->get()
                                      ->toArray();

        $list                   = Home::where('status', '=', 1)
                                      ->where('is_top', '=', 0)
                                      ->orderBy('is_top', 'desc')
                                      ->orderBy('update_time', 'desc')
                                      ->get()
