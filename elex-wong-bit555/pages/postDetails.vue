<template>
  <div class="post-details">
    <div class="post-details-container">
      ...
      <div class="post-details-subscribe" v-if="!isLogin">
        <div class="post-details-subscribe-title">
          {{ $t("signupformore") }}
        </div>
        <div class="post-details-subscribe-btn" @click="subcribe">
          {{ $t("subscribe") }}
        </div>
      </div>
...
export default {
  ...
  methods: {
    initialize() {
      ...
      var time = 1;
      //check if not token delay the api called
      if (!token) time = 200;
    ...
    getEventDetail() {
      this.$api
        .requestByPost("Event/getDetail", { id: this.id })
        .then((result) => {
          if (result.status) {
            this.content = result.data;
          } else {
