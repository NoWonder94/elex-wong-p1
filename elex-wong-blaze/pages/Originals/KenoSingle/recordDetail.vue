<template>
  <div>
    ...
    >
      <div class="header">
        <span>{{ $tt("KenoSingle.Details") }}</span>
      ...
      <div class="recordItem">
        <div class="amount item">
          ...
          <div class="bottom">{{ itemInfo.money }} JB</div>
        </div>
        <div class="amount item">
          <div class="top">
        ...
        <div class="amount item">
          <div class="top">
            ...
            <div>{{ $tt("Bet.Profit") }}</div>
          </div>
          <div class="bottom">{{ itemInfo.winMoney }} JB</div>
        </div>
      </div>
      <div class="result">{{ $tt("Bet.Result") }}</div>
      <div class="resultBox">
...
export default {
  ...
  mounted() {
    let position =
      (JSON.parse(this.itemInfo.content).number &&
        JSON.parse(this.itemInfo.content).number.split(",")) ||
      [];
    this.list = Array(40).fill(require("./img/square1.png"));
    position.forEach(
      (index) => (this.list[Number(index) - 1] = require("./img/checked.png"))
    );
  },
  methods: {
    dialogClose() {
