...
class BetHourService {
     ...
     */
    public function selectList($orgId, $page = 1, $pageSize = 50, $param = array(), $order = BetHourModel::ORDER_TIMED_DESC)
    {
        $queryList = BetHourModel::dbTable('d');
        ...
        // 排序
        switch ($order) {
            case BetHourModel::ORDER_TIMED_ASC:
                $queryList->orderBy('d.timed', 'asc');
                break;
            case BetHourModel::ORDER_TIMED_DESC:
    ...
    {
        return BetHourModel::dbTable('d')->select(DB::raw($this->makeSumTotalFields()))
            ->where($where)
            ->get()
            ->shift();
    }
