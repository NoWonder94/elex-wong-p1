...
class BetMonthService {
     ...
     */
    public function selectList($orgId, $page = 1, $pageSize = 50, $param = array(), $order = BetMonthModel::ORDER_TIMED_DESC)
    {
        $queryList = BetMonthModel::dbTable('d');
        ...
        // 排序
        switch ($order) {
            case BetMonthModel::ORDER_TIMED_ASC:
                $queryList->orderBy('d.timed', 'asc');
                break;
            case BetMonthModel::ORDER_TIMED_DESC:
    ...
    {
        return BetMonthModel::dbTable('d')->select(DB::raw($this->makeSumTotalFields()))
            ->where($where)
            ->get()
            ->shift();
    }
