<?php
namespace App\Services\Chart\CoinsPlay\Stats;

use Illuminate\Support\Facades\DB;
use App\Models\Play\Stats\AgencyModel as StatsAgencyModel;
use App\Models\Play\Stats\AgencyGameModel as StatsAgencyGameModel;
...
class ChartService
{
    ...
    protected function sumCoinTotal($where)
    {
        return StatsAgencyModel::dbTable('d')->select(DB::raw(implode(',', [
            ...
            'IFNULL(SUM(robot_gain), 0) as robot_gain',
            'IFNULL(SUM(amount), 0) as amount',
            'IFNULL(SUM(tree_bet_num), 0) as tree_bet_num',
            'IFNULL(SUM(tree_income), 0) as tree_income',
            'IFNULL(SUM(tree_robot_gain), 0) as tree_robot_gain',
