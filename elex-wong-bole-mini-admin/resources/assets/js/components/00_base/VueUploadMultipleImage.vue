<template>
    <div class="img-upload-multiple-layout">
        <div class="left-panel" style="outline:none;">
            <!-- 开始/普通状态下, 显示这个 -->
                ...
                <div v-else class="image-center position-absolute display-flex flex-column justify-content-center align-items-center" @dragover.prevent="onDragover">
                    <div>
                        <svg class="image-icon-drag" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path d="M383.6 229l-.5 1.5.7 1.7c-.2-1.1-.2-2.2-.2-3.2zm-119.7-5.4l-.3 1.4.6 1.3c-.2-.8-.3-1.8-.3-2.7zm62.4 3.8l-.2 1 .5 1.1-.3-2.1z">
                            </path>
            ...
            <div class="image-container position-relative text-center image-list" v-else>
                <div class="preview-image full-width position-relative cursor-pointer" @click="openGallery(currentIndexImage)">
                    <div class="image-overlay position-relative full-width full-height">
                    </div>
                    <div class="image-overlay-details full-width">
                        <svg class="icon-overlay" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                            <path d="M283.9 186.4h-64.6l-.4-71.1c-.1-8.8-7.2-15.9-16-15.9h-.1c-8.8.1-16 7.3-15.9 16.1l.4 70.9h-64.4c-8.8 0-16 7.2-16 16s7.2 16 16 16h64.6l.4 71.1c.1 8.8 7.2 15.9 16 15.9h.1c8.8-.1 16-7.3 15.9-16.1l-.4-70.9h64.4c8.8 0 16-7.2 16-16s-7.1-16-16-16z">
                            </path>
                ...
                <div class="image-bottom display-flex position-absolute full-width align-items-center justify-content-between" :class="!showPrimary && 'justify-content-end'">
                    <div class="image-bottom-left display-flex align-items-center" v-if="showPrimary">
                        <div class="display-flex align-items-center" v-show="imageDefault">
                            <span class="image-primary display-flex align-items-center">
                                <svg class="image-icon-primary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                    ...
                                    <path fill="#FFF" d="M216.7 350.9h-.1c-5.1 0-9.9-2.1-13.4-5.7l-74.2-76c-7.4-7.5-7.2-19.5.4-26.8 7.5-7.4 19.5-7.2 26.8.4L217 305l139.7-138.5c7.5-7.4 19.5-7.4 26.8.1s7.4 19.5-.1 26.8l-153.2 152c-3.7 3.5-8.5 5.5-13.5 5.5z">
                                    </path>
                                </svg>
                                {{ primaryText }}
                            </span>
                        ...
                        <a class="text-small mark-text-primary cursor-pointer" @click.prevent="markIsPrimary(currentIndexImage)" v-show="!imageDefault">
                            {{ markIsPrimaryText }}
                        </a>
                    </div>
                    <div class="display-flex">
                        <label class="image-edit display-flex cursor-pointer" :for="idEdit">
                            <svg class="image-icon-edit" xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512">
                        ...
                        <a class="image-delete display-flex cursor-pointer" @click.prevent="deleteImage(currentIndexImage)">
                            <svg class="image-icon-delete" xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512">
                                <path d="M448 64h-96V0H159.9l.066 64H32v32h32v416h384V96h32V64h-32zM192 32h128v32H192V32zm224 448H96V96h320v384zM192 160h32v256h-32V160zm96 0h32v256h-32V160z">
                                </path>
                            </svg>
                        </a>
                    </div>
        ...
        <div class="right-panel">
            <div class="uploaded-img-list">
                <div class="message-no-data" v-if="!images.length">
                    {{ $t('message.empty.data') }}
                </div>
                <div class="uploaded-img" v-for="(image, index) in images" :key="index" @click="changeHighlight(index)" v-else>
                    <img :src="image.path">
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            alertErrorMsg() {
                ...
                this.$message({
                    type: 'error',
                    message: this.$t('message.error.imgAmountMaxLimit') + ': ' + this.maxImage,
                    showClose: true,
                    onClose: function () {
            ...
            generateImgName() {
                ...
                for (var i = length; i > 0; --i) {
                    name += char[Math.round(Math.random() * (char.length - 1))];
                }
            ...
            preUploadImg(name, img) {
                let formData = new FormData();
                formData.append('name', name);
                formData.append('img', img);

                axios.post('/media/uploadImg', formData, config).then((response) => {
            ...
            createImage(file) {
                ...
                reader.onload = (e) => {
                    ...
                    if (dataURI) {
                        //this.preUploadImg(file.name, dataURI);
                        var name = this.generateImgName();
                        this.preUploadImg(name, dataURI);
                ...
                reader.onload = (e) => {
                    ...
                    if (dataURI) {
                        ...
                        if (this.images.length && this.images[this.currentIndexImage]) {
                            ...
                            this.images[this.currentIndexImage].name = name;
...
<style lang="scss" type="text/css">
    @import "../../../scss/bole_mini/Component/VueUploadMultipleImage.scss";
</style>