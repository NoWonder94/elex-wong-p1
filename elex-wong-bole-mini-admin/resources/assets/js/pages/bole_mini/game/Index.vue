<template>
	<div class="game" v-loading="loading">
                ...
                <div class="action">
                    <el-form :inline="true" :model="searchForm">
                        ...
                        <div class="action-list">
                            <div class="button-add" @click="showModal('addForm')" v-if="authority.indexOf(35) >= 0">
        	...
        	<div class="table-template">
	        	<b-table :items="resultData" :fields="fields" :per-page="amountPerPage" :current-page="currentPage" :filter="filter" @filtered="onFiltered">
                    <template slot-scope="row" slot="icon">
                        <img :src="row.item.icon" @click="showImgModal(row.item.icon)">
                    </template>
                    <template slot-scope="row" slot="img">
                        <img :src="row.item.img" @click="showImgModal(row.item.img)">
                    </template>
                    ...
                    <template slot-scope="row" slot="status">
                        <div class="status">
                            ...
                            <div class="status-coming-soon" v-else-if="row.item.status == 2">
                                {{ $t('status.comingSoon') }}
                            </div>
	            	...
	            	<template slot-scope="row" slot="actions">
	                    <b-button class="edit" @click.stop="showModal('updateForm', row.item)" v-if="authority.indexOf(36) >= 0">
	                        {{ $t('button.edit') }}
	                    </b-button>
	                    <b-button class="delete" @click.stop="showModal('deleteForm', row.item)" v-if="authority.indexOf(37) >= 0">
	                        {{ $t('button.delete') }}
	                    </b-button>
	            	</template>
        ...
        <item-detail :visible.sync="isDialogVisible" :formType="formType" :detail="detail" :gameCategoryList="gameCategoryList">
        </item-detail>
	</div>
</template>
<script>
		...
		data() {
			return {
                ...
                searchForm: {
	          	...
	          	fields: [
	          		{ key: 'game_category_name', label: '游戏类型', sortable: true },
                    { key: 'icon', label: '图标' },
	          		{ key: 'img', label: '图片' },
	          		{ key: 'chinese.title', label: '标题', sortable: true },
                    { key: 'sort', label: '顺序', sortable: true },
		...
		methods: {
			...
			init() {
                axios.get('/game/getList').then((response) => {
                    ...
                    this.gameCategoryList = response.data.resp_data.gameCategoryList;
                    this.loading = false;
				}).catch(error => {
...
<style lang="scss" type="text/css">
	@import "../../../../scss/bole_mini/Game.scss";
</style>