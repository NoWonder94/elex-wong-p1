<template>
	<div class="info" v-loading="loading">
                ...
                <div class="action">
                    <el-form :inline="true" :model="searchForm">
                        ...
                        <div class="action-list">
                            <div class="button-add" @click="showModal('addForm')" v-if="authority.indexOf(8) >= 0">
        	...
        	<div class="table-template">
	        	<b-table :items="resultData" :fields="fields" :per-page="amountPerPage" :current-page="currentPage" :filter="filter" @filtered="onFiltered">
	            	...
	            	<template slot-scope="row" slot="actions">
	                    <b-button class="edit" @click.stop="showModal('updateForm', row.item)" v-if="authority.indexOf(9) >= 0">
	                        {{ $t('button.edit') }}
	                    </b-button>
	                    <b-button class="delete" @click.stop="showModal('deleteForm', row.item)" v-if="authority.indexOf(10) >= 0">
	                        {{ $t('button.delete') }}
	                    </b-button>
	            	</template>
        ...
        <item-detail :visible.sync="isDialogVisible" :formType="formType" :detail="detail" :infoCategoryList="infoCategoryList">
        </item-detail>
	</div>
</template>
<script>
	...
	export default {
		...
		data() {
			return {
                ...
                searchForm: {
	          	...
	          	fields: [
	          		{ key: 'info_category_name', label: '资讯类型', sortable: true },
	          		{ key: 'img', label: '图片' },
		...
		methods: {
			...
			init() {
                axios.get('/info/getList').then((response) => {
                    this.resultData = response.data.resp_data.list;
                    this.totalRows = response.data.resp_data.totalCount;
                    this.infoCategoryList = response.data.resp_data.infoCategoryList;
                    this.loading = false;
				}).catch(error => {
...
<style lang="scss" type="text/css">
	@import "../../../../scss/bole_mini/Info.scss";
</style>