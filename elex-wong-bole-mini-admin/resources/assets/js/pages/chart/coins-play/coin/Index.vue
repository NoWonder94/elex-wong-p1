    ...
    <div class="page-content" v-loading="loading">
        ...
        <div class="list-space"></div>
            <el-table style="width: 100%"
                ...
                <el-table-column min-width="100" :label="$t('chart.coin.thLable.platformCommission')">
                    <template slot-scope="scope">{{ scope.row.income > 0 ? '+' : '' }}{{ scope.row.income | numeral('0,0.[0000]') }}</template>
                </el-table-column>
                <el-table-column min-width="110" :label="$t('chart.coin.thLable.AIPnL')">
                    <template slot-scope="scope">{{ scope.row.robot_gain > 0 ? '+' : '' }}{{ scope.row.robot_gain | numeral('0,0.[0000]') }}</template>
                </el-table-column>
                <el-table-column min-width="100" :label="$t('chart.coin.thLable.total')">
                ...
                <el-table-column min-width="170" :label="$t('chart.coin.thLable.platformCommissionChild')">
                    <template slot-scope="scope">{{ scope.row.tree_income > 0 ? '+' : '' }}{{ scope.row.tree_income | numeral('0,0.[0000]') }}</template>
                </el-table-column>
                <el-table-column min-width="190" :label="$t('chart.coin.thLable.AIPnLChild')">
                    <template slot-scope="scope">{{ scope.row.tree_robot_gain > 0 ? '+' : '' }}{{ scope.row.tree_robot_gain | numeral('0,0.[0000]') }}</template>
                </el-table-column>
                <el-table-column min-width="160" :label="$t('chart.coin.thLable.totalChild')">
    ...
    export default {
        ...
        methods: {
            ...
            getDataList() {
                ...
                axios.get('/chart/coins-play/getCoinList', {
