<template>
    <div class="page-content" v-loading="loading">
        ...
        <el-table style="width: 100%"
            ...
            <el-table-column min-width="100" :label="$t('chart.game.thLable.platformCommission')">
                <template slot-scope="scope">{{ scope.row.income > 0 ? '+' : '' }}{{ scope.row.income | numeral('0,0.[0000]') }}</template>
            </el-table-column>
            <el-table-column min-width="110" :label="$t('chart.game.thLable.AIPnL')">
                <template slot-scope="scope">{{ scope.row.robot_gain > 0 ? '+' : '' }}{{ scope.row.robot_gain | numeral('0,0.[0000]') }}</template>
            </el-table-column>
            <el-table-column min-width="100" :label="$t('chart.game.thLable.total')">
            ...
            <el-table-column min-width="170" :label="$t('chart.game.thLable.platformCommissionChild')">
                <template slot-scope="scope">{{ scope.row.tree_income > 0 ? '+' : '' }}{{ scope.row.tree_income | numeral('0,0.[0000]') }}</template>
            </el-table-column>
            <el-table-column min-width="190" :label="$t('chart.game.thLable.AIPnLChild')">
                <template slot-scope="scope">{{ scope.row.tree_robot_gain > 0 ? '+' : '' }}{{ scope.row.tree_robot_gain | numeral('0,0.[0000]') }}</template>
            </el-table-column>
            <el-table-column min-width="160" :label="$t('chart.game.thLable.totalChild')">
    ...
    export default {
        ...
        methods: {
            ...
            getDataList() {
                ...
                axios.get('/chart/coins-play/getGameList', {
            ...
            getSummaries() {
                ...
                sums.push((this.dataSumTotal.income > 0 ? '+' : '') + this.$options.filters.numeral(this.dataSumTotal.income, '0,0.[0000]'));
                sums.push((this.dataSumTotal.other_income > 0 ? '+' : '') + this.$options.filters.numeral(this.dataSumTotal.other_income, '0,0.[0000]'));
                sums.push((this.dataSumTotal.robot_gain > 0 ? '+' : '') + this.$options.filters.numeral(this.dataSumTotal.robot_gain, '0,0.[0000]'));
                ...
                sums.push((this.dataSumTotal.tree_income > 0 ? '+' : '') + this.$options.filters.numeral(this.dataSumTotal.tree_income, '0,0.[0000]'));
                sums.push((this.dataSumTotal.other_tree_income > 0 ? '+' : '') + this.$options.filters.numeral(this.dataSumTotal.other_tree_income, '0,0.[0000]'));
                sums.push((this.dataSumTotal.tree_robot_gain > 0 ? '+' : '') + this.$options.filters.numeral(this.dataSumTotal.tree_robot_gain, '0,0.[0000]'));
