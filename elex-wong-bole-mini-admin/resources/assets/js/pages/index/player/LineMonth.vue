<template>
    <div class="chart-line-card" v-loading="loading">
        ...
        <div v-else class="chart-content chart-line" id="chart-line-player-month"></div>
    </div>
</template>
    ...
    export default {
        ...
        computed: {
            lineOption: function () {
                let option = {
                    ...
                    xAxis: {
                        type: 'category',
                        name: this.$t('index.line-player.month'),
                        data: []
                    },
                    yAxis: {
                        type: 'value',
        ...
        watch: {
            activeTime: function (n, o) {
                // 初始化数据
                if (this.activeTime == 'month' && this.activeName == 'player' && !this.dataList.length) {
                    this.getDataList();
                }
            },
        ...
        methods: {
            getDataList() {
                ...
                this.loading = true;
                axios.get('/index/month/getPlayerLine').then((response) => {
            ...
            initChartLine() {
                // 绘制图表
                this.chartLine = echarts.init(document.getElementById('chart-line-player-month'));
