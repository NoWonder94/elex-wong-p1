        ...
        <tpl:list>
            <search>
                <row>
					...
					<btn css="btn-info" id="exportBtn" auth="export" label="导出"></btn>
                </row>
            </search>
            <content>
                <table>
                    <columns>
						...
						<actions width="150">
							...
							<action css="text-success" label="播放">
                                <attrs>
									<click>$.PlayRecord('{!! $list_item['record'] . '?' . UTC_TIME !!}')</click>
                                </attrs>
                            </action>
							...
							<action label="重新备份" css="text-danger">
                                <attrs>
                                    <click>$.ResetSaveRecord('{!! $list_item['sid'] !!}', '{!! $list_item['api_type'] !!}')</click>
                                </attrs>
                            </action>
							@endif
						</actions>
...
jQuery(function($){
	$("#exportBtn").click(function(){
		$("#searchForm").attr('target','_blank')
					    .attr('action', "{!! Helper::url('CallLog/export') !!}")
						.submit();
	});
	
	$("#searchBtn").click(function(){
		$("#searchForm").attr('target','_self')
					    .attr('action', "{!! Helper::url('CallLog/index') !!}")
						.submit();
	});
	...
	$.ResetSaveRecord = function(sid, type) {
		var url = '{{ Helper::url("callback#AAAAA/forceSendRecordToS3") }}'.replace('AAAAA', type);
		$.ajaxPost(url, { CallSid:sid }, function(result){
