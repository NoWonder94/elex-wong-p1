        ...
        <tpl:list>
            <search>
                <row>
                    ...
                    <item name="proxy" label="代理账户"></item>
                    <item label="性别">
                ...
                <row style="margin-top:10px;">
                    <item label="最后登录">
                        <tpl:daterange data="$_search_args" begin_name="login_begin_time" end_name="login_end_time"></tpl:daterange>
                    </item>
                    <item label="最后存款">
                        <tpl:daterange data="$_search_args" begin_name="deposit_begin_time" end_name="deposit_end_time"></tpl:daterange>
                    </item>
                    <btn type="search"></btn>
                </row>
            </search>
                ...
                <table>
                    <columns>
                        <column code="name" label="用户信息" align="left">
                            用户名：{{ $list_item['name'] }} {{ $list_item['user_group'] }}<br/>
                            姓　名：{{ $list_item['real_name'] }} {{ $list_item['gender'] }}
                        </column>
                        <column type="time" label="注册/登录日期" width="190" align="left">
                            注册：{{ Time::toDate($list_item['reg_time']) }}<br/>
                            登录：{{ Time::toDate($list_item['login_time']) }}
                        </column>
                        <column label="最后存款" width="190" align="left">
                            金额：{{ $list_item['last_deposit_money'] }}<br/>
                            日期：{{ Time::toDate($list_item['last_deposit_time']) }}
                        </column>
                        <column type="time" label="最后电话/短信时间" width="190" align="left">
                            电话：{{ Time::toDate($list_item['last_call_time']) }}<br/>
                            短信：{{ Time::toDate($list_item['last_sms_time']) }}
                        </column>
                        <actions width="160">
	...
	$.plivoSetup = function() {
        $.ajaxPost(SITE_URL + '/User/token', '', function(result){
            if (result.status) {
				...
				plivoWebSdk.client.login(result.data.endpoint_user, result.data.endpoint_password);
				$(".call-btn").show();
	...
	$.SendCall = function(id, name) {
        $.ajaxPost(SITE_URL + '/User/call', { user_id: id }, function(result){
            if (result.status) {
				...
				plivoWebSdk.client.call(logId);
                modalBox.find('.btn-default').click(function(){
                    plivoWebSdk.client && plivoWebSdk.client.callSession && plivoWebSdk.client.hangup();
                });
				...
				plivoWebSdk.client.on('onCallFailed',function(){
					console.log('onCallFailed');
					modalBox.find('.call-status').html('通话失败');
                    modalBox.find('.btn-primary').show();
                    modalBox.find('.btn-default').hide();
				});
				
				plivoWebSdk.client.on('onCallTerminated',function(){
