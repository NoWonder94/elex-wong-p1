...
class Strings {
	 ...
	 * @return string
	 */
	static public function clearSymbol($str){
		static $symbols = NULL;
		if($symbols === NULL){
			$symbols = file_get_contents('../resources/table/symbol.table');
			$symbols = explode("\r\n",$symbols);
		}
		return str_replace($symbols,'',$str);
	}

    static public function strToChar($str){
	...
	static public function strToUnicode($str, $pre = ''){
		$str = self::clearSymbol(strtolower($str));
		$arr = array();
		$str_len = mb_strlen($str, 'UTF-8');
		for ($i = 0; $i < $str_len; $i++){
	 ...
	 * @return void
	 */
	static public function utf8ToUnicode($char){
		switch(strlen($char)){
			case 1:
				return ord($char);
			case 2:
