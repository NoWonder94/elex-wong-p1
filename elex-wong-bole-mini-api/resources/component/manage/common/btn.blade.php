...
switch($attrs['type']){
    case 'submit':
        ...
        $label    = empty($label) ? Lang::get('root::tpl.submit') : $label;
        $btnType  = 'submit';
		if (!isset($attrs['auth'])) {
			$attrs['noauth'] = true;
		}
    ...
    case 'search':
        $css      = empty($css) ? 'btn-success' : $css;
        $label    = empty($label) ? Lang::get('root::tpl.search') : $label;
        $btnType  = 'submit';
		if (!isset($attrs['auth'])) {
			$attrs['noauth'] = true;
		}
    ...
    case 'add':
        ...
        $label  = empty($label) ? Lang::get('root::tpl.add') : $label;
        $url    = empty($url) ? '{!! Helper::url(APP_CONTROLLER_NAME."/add") !!}' : $url;
    break;
    case 'refresh':
        $css    = empty($css) ? 'btn-info' : $css;
        $label  = empty($label) ? Lang::get('root::tpl.refresh') : $label;
    ...
    case 'export':
        $css    = empty($css) ? 'btn-info' : $css;
        $label  = empty($label) ? Lang::get('root::tpl.export') : $label;
        $url    = empty($url) ? '{!! Helper::url(APP_CONTROLLER_NAME."/export") !!}' : $url;
    break;
    case 'import':
        $css    = empty($css) ? 'btn-warning' : $css;
        $label  = empty($label) ? Lang::get('root::tpl.import') : $label;
