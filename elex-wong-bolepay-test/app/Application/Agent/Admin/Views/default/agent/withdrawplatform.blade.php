...
<tpl:fitem label="代付费用设置" bcss="min-list">
    <attrs>
        <tip>服务费=基础服务费率 + 加收服务费率，按代付金额百分比 + 基础笔费 + 加收笔费</tip>
    </attrs>
    <table class="table table-bordered table-th-bg table-vmiddle" style="width: auto; text-align: center;">
        ...
        @foreach($withdrawplatform as $payment_id => $payment)
            ...
            <tr>
                ...
                <td>
                    <span id="agentWithdrawRate{{ $payment_id }}">{{ $payment['rate'] + $agent_withdrawplatform['rate'] }}% + {{$payment['rate_money'] + $agent_withdrawplatform['rate_money']}}</span>
                </td>
            </tr>
            @endif
        @endforeach
