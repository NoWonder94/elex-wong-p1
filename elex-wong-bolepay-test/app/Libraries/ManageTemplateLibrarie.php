...
class ManageTemplateLibrarie extends TemplateLibrarie {
    ...
    protected function preListTable($content, &$pattrs) {
        return $this->preTags($content, [
                        ...
                        '<tpl:listDetails', '</tpl:listDetails>',
     ...
     * @return string          格式化后的内容
     */
    protected function parseListPanels($attrs, $content, &$pattrs) {
        return $this->formatTpl('list.panels', ['attrs' => $attrs, 'content' => $content, 'List' => $pattrs['List']]);
    }
    ...
    protected function parseListHeaderActions($attrs, $content, &$pattrs) {
        $attrs['label'] = Lang::get('admin.handler');
        return $this->formatTpl('list.thead-item', ['attrs' => $attrs, 'content' => $content]);
    }
    ...
    protected function preListDetails($content, &$pattrs) {
        $pattrs['ListTable']['details'] = $content;
        return '';
    }
