...
class YlLibrarie extends BaseLibrarie {
    ...
    public function notify($data) {
        ...
        try {
            $status = $data['orderstatus'] == 1 ? 1 : -1;
            if($status == 2){
                $remark = '转账流水号：' . $data['order_id'];
            }else{
                $remark = $data['apiremark'];
            }
            ApiWithdrawService::instance()->pay($data['out_biz_no'], $money,$status,$remark);
        } catch (\Exception $e) {
            return 'error';
        }
        return 'success';
    ...
    public function create(&$withdraw){
        ...
        $data['recipients'] = preg_replace("/\s|　/","",$withdraw->bank_no);
        $data['name'] = preg_replace("/\s|　/","",$withdraw->payee);
        $data['amount'] = floatval($withdraw->money);
