...
class Collection extends \Illuminate\Database\Eloquent\Collection {
    ...
    public function getKeyListArray($key, \Closure $formatData = null) {
        $dictionary = [];
        foreach ($this->items as $value) {
            $dictionary[$value->$key] = $value->toArray();
            if ($formatData) {
                call_user_func_array($formatData, [&$dictionary[$value->$key], $value]);
            }
        }
        return $dictionary;
    }

    public function geListArray(\Closure $formatData = null) {
        $dictionary = [];
        foreach ($this->items as $key => $value) {
            $dictionary[$key] = $value->toArray();
            if ($formatData) {
                call_user_func_array($formatData, [&$dictionary[$key], $value]);
            }
        }
        return $dictionary;
    }
