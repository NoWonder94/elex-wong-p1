...
$bcss       = isset($attrs['bcss']) ? $attrs['bcss'] : '';
$bstyle     = isset($attrs['bstyle']) ? $attrs['bstyle'] : '';
...
$locales    = isset($attrs['locales']) ? true : false;
...
$attr .= ' data-fv-blank="true"';

if (!empty($required)) {
    $attr .= ' data-fv-notempty="true"';
    if (!isset($attrs['data-fv-notempty-message'])) {
        $attr .= ' data-fv-notempty-message="' . Lang::get('admin.please_enter') . Tpl::output($label) . '"';
    }
}
?>
@if($name)
...
<div 
 @if($pid) id="{!! Tpl::output($pid) !!}" @else id="{!! Tpl::output($id) !!}-form-item" @endif class="form-group {!! Tpl::output($pcss) !!}" style="{!! Tpl::output($pstyle) !!}" >
    ...
    @if($search != 1)
    <div class="{!! Tpl::output($bcss) !!}" style="{!! Tpl::output($bstyle) !!}">
    @endif
    @if(empty($content) || $append)
        @if($type == 'imagebtn')
        <div class="image-btn-box">
            <a class="img-upload-btn btn {!! Tpl::output($css) !!}" style="{!! Tpl::output($style) !!}" {!! Tpl::output($attr) !!} href="javascript:;">{{ Lang::get('admin.upload_image') }}</a>
        ...
        @else
            @if($locales)
                @foreach(Config::get('app.locales') as $locale)
                    <php>
                        $_input_data_locale = $_input_data['{{ $locale }}'];
                    </php>
                    <div class="input-group">
                      <span class="input-group-addon">{{ $locale }}</span>
                      <input type="text" name="{!! Tpl::output($name) !!}[{{ $locale }}]" id="{!! Tpl::output($id) !!}_{{ $locale }}" class="form-control {!! Tpl::output($css) !!}" style="{!! Tpl::output($style) !!}" placeholder="{!! Tpl::output($placeholder) !!}" {!! Tpl::output($attr) !!} value="@{{ $_input_data_locale }}" />
                    </div>
                @endforeach
            @else
