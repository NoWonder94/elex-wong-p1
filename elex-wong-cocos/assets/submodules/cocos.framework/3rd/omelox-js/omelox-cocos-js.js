 ...
 * omelox客户端主要接口简介：
    ...
    非首次初始化omelox的时候，一定要等上次的disconnect后再初始化
...
(function () {
  ...
  Emitter.prototype.on =
    Emitter.prototype.addEventListener = function (event, fn, caller) {
  ...
  Emitter.prototype.once = function (event, fn, caller) {
    var self = this;
    this._callbacks = this._callbacks || {};

    function on() {
      self.off(event, on, caller);
      // fn.apply(this, arguments);
      fn.call(caller, event, arguments);
    }
  ...
  Emitter.prototype.off =
    ...
    Emitter.prototype.removeEventListener = function (event, fn, caller) {
      this._callbacks = this._callbacks || {};
      ...
      for (var i = 0; i < callbacks.length; i++) {
        cb = callbacks[i];
        if ((cb === fn || cb.fn === fn) && caller == null || caller && cb.caller == caller) {
  ...
  Emitter.prototype.emit = function (event) {
    ...
    if (callbacks) {
      ...
      for (var i = 0, len = callbacks.length; i < len; ++i) {
        cb = callbacks[i];
        cb.fn.apply(cb.caller, args);
        // cb.fn.call(cb.caller, event, args);
      }
    }
...
(function (exports, ByteArray, global) {
  ...
  Protocol.strdecode = function (buffer) {
    ...
    while (offset < end) {
      ...
      // array.push(charCode);
      tempStr += String.fromCharCode.apply(null, [charCode]);
    }
...
(function (expParam) {
  var Client = function () {
    ...
    omelox.connect = function (params, cb) {
      initCallback = cb;
    ...
    var defaultDecode = omelox.decode = function (data) {
      //probuff decode
    ...
    var defaultEncode = omelox.encode = function (reqId, route, msg) {
    ...
    var connect = function (params, url, cb) {
      ...
      var onerror = function (event) {
        omelox.emit(Event.onIOError, "网络错误");
        console.error('socket error: ', event);
      };
      var onclose = function (event) {
        if (!!params.reconnect) {
          if (reconnectAttempts < maxReconnectAttempts) {
            omelox.emit(Event.onReconnecting, "重连中");
            reconnect = true;
            reconnectAttempts++;
            ...
            omelox.emit(Event.onReconnectFail, "重连失败");
          }

        } else {
          omelox.emit(Event.onDisconnect, "网络断开");
        }
      ...
      omelox.socket = socket;
    };

    omelox.disconnect = function (code, reason, cb) {
      disconnectCb = cb;
      if (socket) {
        if (socket.disconnect) socket.disconnect(code, reason);
        if (socket.close) socket.close(code, reason);
        logger.warn('omelox disconnect', code, reason);
        socket = null;
      }
    ...
    var heartbeatTimeoutCb = function () {
      ...
      if (gap > gapThreshold) {
        ...
        omelox.emit(Event.onDisconnect, '心跳超时');
        omelox.disconnect(-1, 'heartbeat timeout');
      }
    };
    ...
    var handshake = function (data) {
      ...
      if (data.code === RES_OLD_CLIENT) {
        omelox.emit(Event.onIOError, 'client version not fullfill');
        return;
      }

      if (data.code !== RES_OK) {
        omelox.emit(Event.onIOError, 'handshake fail');
        return;
      }
      ...
      if (!!reconnect) {
        omelox.emit(Event.onReconnected, "重连成功");
        reconnect = false;
      }
