import { _decorator, Component, Node, SpriteFrame, Prefab, warn, Material } from 'cc';
const { ccclass, property, executeInEditMode, menu } = _decorator;

@ccclass("CCReferenceCollector")
@executeInEditMode
@menu("自定义组件/UI/CCReferenceCollector(节点集合容器)")
export default class CCReferenceCollector extends Component {
    /** 自己加载组件名称前缀 */
    static autoLoadNodePrefix: string = 'CC_';
    @property({
        type: [Node],
        tooltip: "手动添加Node集合",
    })
    ...
    onFocusInEditor() {
        warn('onFocusInEditor', this.node.name)
        if (CC_EDITOR) {
            this.autoLoadNode();
        }
    }
    ...
    onLostFocusInEditor() {
        warn('onLostFocusInEditor', this.node.name)
        if (CC_EDITOR) {
            this.autoLoadNode();
        }
    }
    ...
    public get<T extends any>(key: string, component?: any): T {
        ...
        if (!v) {
            logger.warn('CCReferenceCollector 获取节点失败', key);
            return null;
        }
    ...
    public getRes<T extends any>(key: string, type: any): T {
        ...
        if (!v) {
            logger.warn('CCReferenceCollector 获取资源失败', key);
            return null;
        }
    ...
    private autoLoadNode() {
        ...
        this.loadNode(this.node, "", (node) => {
            if (node.getComponent(CCReferenceCollector)) {
                if (node.name.startsWith(CCReferenceCollector.autoLoadNodePrefix)) {
