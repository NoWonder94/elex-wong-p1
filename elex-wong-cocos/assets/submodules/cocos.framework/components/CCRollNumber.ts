import { _decorator, Label, Component, Enum, math, EventHandler } from 'cc';
 ...
 */
@ccclass('CCRollNumber')
@menu('自定义组件/UI/CCRollNumber(切换页面)')
export default class CCRollNumber extends Component {
    @property({
        type: Label,
    ...
    updateLabel() {
        ...
        // logger.warn('scroll set label', this.label.string, string);
        if (this.label) {
    ...
    update(dt: number) {
        ...
        // logger.warn('scroll', this.value, this.targetValue);
        this.value = math.lerp(this.value, this.targetValue, this.lerp);
        this.updateLabel();
        if (Math.abs(this.value - this.targetValue) <= 0.1) {
            this.value = this.targetValue;
            this.label.string = this.targetValue + '';
            this.isScrolling = false;
