 ...
 * @date 2020/12/9
 * @doc 列表组件, 来源https://github.com.cnpmjs.org/gh-kL/cocoscreator-list.git
 * @end
 ******************************************/
...
export default class CCList extends Component {
    ...
    public templateType: TemplateType = TemplateType.NODE;
    //模板Item（Node）
    @property({
        type: Node,
        tooltip: DEV && '模板Item',
    ...
    public renderEventFunctionCaller: FunctionCaller;
    public pageEventFunctionCaller: FunctionCaller;

    private onRenderEventListRender(item: any, idx: number) {
        this.renderEventFunctionCaller.exec(item, idx);
    }

    private onPageEventListRender(item: any, idx: number) {
        this.pageEventFunctionCaller.exec(item, idx);
    }

    public setRenderEvent(handler: FunctionCaller) {
        this.renderEventFunctionCaller = handler;
        this.renderEvent.target = this.node;
        this.renderEvent.component = 'CCList';
        this.renderEvent.handler = 'onRenderEventListRender';
        this.renderEvent.customEventData = '';
    }

    public setPageEvent(handler: FunctionCaller) {
        this.pageEventFunctionCaller = handler;
        this.pageChangeEvent.target = this.node;
        this.pageChangeEvent.component = 'CCList';
        this.pageChangeEvent.handler = 'onPageEventListRender';
        this.pageChangeEvent.customEventData = '';
    }
    ...
    public selectedEventFunctionCaller: FunctionCaller;

    private onSelectedEventListRender(item: any, idx: number) {
        this.selectedEventFunctionCaller.exec(item, idx);
    }

    public setSelectedEvent(handler: FunctionCaller) {
        this.selectedEventFunctionCaller = handler;
        this.selectedEvent.target = this.node;
        this.selectedEvent.component = 'CCList';
        this.selectedEvent.handler = 'onSelectedEventListRender';
        this.selectedEvent.customEventData = '';
    }
    ...
    set selectedId(val: number) {
        ...
        switch (t.selectedMode) {
            ...
            case SelectedType.MULT: {
                item = t.getItemByListId(val);
                if (!item) return;
                let listItem = item.getComponent(CCListItem);
                if (t._selectedId >= 0) t._lastSelectedId = t._selectedId;
                t._selectedId = val;
    ...
    setTemplateItem(item: any) {
        ...
        let com: any = item.getComponent(CCListItem);
        let remove = false;
        if (!com) remove = true;
        // if (com) {
    ...
    checkInited(printLog: boolean = true) {
        if (!this._inited) {
            if (printLog) console.error('CCList initialization not completed!');
            return false;
        }
        return true;
    }
        ...
        if (this._virtual) {
            ...
            if (this._forceUpdate || haveDataChange) {
                ...
                if (this.frameByFrameRenderNum > 0) {
                    ...
                    //直接渲染
                    this._lastDisplayData = [];
                    // cc.log('CCList Display Data II::', this.displayData);
    ...
    _createOrUpdateItem(data: any) {
        ...
        if (!item) {
            ...
            let listItem: CCListItem = item.getComponent(CCListItem);
    ...
    _updateListItem(listItem: CCListItem) {
        if (!listItem) return;
