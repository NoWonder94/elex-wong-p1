...
import { _decorator, Component, Node, log, Enum } from 'cc';
...
export default class VMCompsEdit extends Component {
    ...
    searchComponent(className: string, state: number = 0) {
        ...
        comps.forEach(v => {
            ...
            if (state <= 3) {
                //区分模板模式路径
                if (tv.templateMode === true) {
                    ext = tv.watchPathArr ? ':[Path:' + tv.watchPathArr.join('|') + ']' : ''
                } else {
                    ext = tv.watchPath ? ':[Path:' + tv.watchPath + ']' : ''
                }
            }
            ...
            switch (state) {
                ...
                case 4://替换指定路径
                    ...
                    if (tv.templateMode === true) {
                        for (let i = 0; i < tv.watchPathArr.length; i++) {
                            const path = tv.watchPathArr[i];
                            tv.watchPathArr[i] = this.replaceNodePath(path, targetPath, replacePath);
                        }
                    } else {
                        tv.watchPath = this.replaceNodePath(tv.watchPath, targetPath, replacePath);
                    }
                    break;
