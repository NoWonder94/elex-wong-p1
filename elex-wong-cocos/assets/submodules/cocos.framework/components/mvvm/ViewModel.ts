...
class ViewModel<T> {
    ...
    private _callback(n: any, o: any, path: string[]): void {
        if (this.active == true) {
            ...
            if (DEBUG_SHOW_PATH) logger.info(">>", n, o, path);
...
class VMManager {
    ...
    /**等同于 director.on */
    bindPath(path: string, callback: any, target?: any, useCapture?: boolean): void {
        path = path.trim(); //防止空格,自动剔除
        if (path == "") {
    ...
    /**等同于 director.off */
    unbindPath(path: string, callback: any, target?: any): void {
        path = path.trim(); //防止空格,自动剔除
        if (path.split(".")[0] === "*") {
            console.error(path, "路径不合法,可能错误覆盖了 VMParent 的onLoad 方法, 或者父节点并未挂载 VMParent 相关的组件脚本");
            return;
