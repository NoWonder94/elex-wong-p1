...
export default class Timer {
    ...
    private _onSpan(): void {
        ...
        if (this._callback) {
            try {
                ...
                logger.warn('警告] Timer._onSpan callback has error', error);
            }
        }
    ...
    startAndBindToNode(parent: Node): Timer {
        // if (parent.getChildByName('timer_a')) {
        //     return;
        // }
        // cc.log("startAndBindToNode", parent)
        let node = new Node();
        // node.name = 'timer_a';
        let component = node.addComponent(Component);
