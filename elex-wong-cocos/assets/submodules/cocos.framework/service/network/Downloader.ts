...
export class Downloader {
    ...
    private constructor() {
        if (!CC_JSB) {
            logger.warn('Downloader is a NATIVE ONLY feature.');
        }
    ...
    downloadFile(item: DownloadItem) {
        ...
        logger.info(`Downloader, downloadFile, url=${requestURL}, downloadingCnt=${this._downloadingCnt}`);
    }
    ...
    private downloadQueueItem() {
        if (!this._downloadQueue || this._downloadQueue.length < 1) {
            logger.info(`Downloader, no queue or all queue item had been downloaded`);
            return;
        }
    ...
    private onFileTaskSuccess(requestURL: string, jsbDownloader, task: DownloadTask) {
        ...
        this._downloadingCnt--;
        logger.info(`Downloader, onFileTaskSuccess, url=${requestURL}, downloadingCnt=${this._downloadingCnt}`);
        this.downloadQueueItem();
    }
    ...
    private onTaskError(requestURL: string, jsbDownloader, task: DownloadTask, errCode: number, errCodeInternal: number, errStr: string) {
        ...
        this.pushJsbDownloader(jsbDownloader);
        this._downloadingCnt--;
        logger.info(`Downloader, onTaskError, url=${requestURL}, downloadingCnt=${this._downloadingCnt}`);
        this.downloadQueueItem();
    }
