...
export class StateMachine {
        ...
        for (const transition of this.transitions) {
            if (transition.from.indexOf(oldState) > -1) {
                ...
                for (const action of transition.action) {
                    const ret = preData === undefined ? action(data) : action(...preData, data);
    ...
    private onMessageReceive(msg: string, data: any) {
        for (const transition of this.transitions) {
            ...
            for (const item of transition.whenChannelWrited) {
                if (item.indexOf('?') > -1) {
                    const reg = new RegExp(item.replace(/\?/g, ' (\\S+) '), 'g');
                    const m = [];
                    let r = null;
    ...
    private mainLoop() {
        if (this.channel.firstMessage()) {
            const [msg, data] = this.channel.read();
            logger.info('mainLoop read=', msg, data)
    ...
    when(conditionOrConditions: Function | string) {
        ...
        const type = typeof conditionOrConditions;
        if (type === 'string') {
    ...
    do(actionOrActions: Function | Function[]) {
        const actions = typeof actionOrActions === 'function' ? [actionOrActions] : actionOrActions;
        if (this.currentFactory === null) {
            this.currentFactory = new Transistion();
        }
    ...
    transition(rule: string) {
        ...
        const arr = rule.split(/(?:\-|\=)\>/).map(item => item.trim());
        if (arr.length !== 2) {
            throw new Error();
        }
        else {
            const [froms, to] = arr;
            const fromArr = froms.split(',').map(item => item.trim());
