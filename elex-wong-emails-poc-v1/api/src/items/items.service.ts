...
export const create2 = async (airdrop: Airdrop,ip:string): Promise<any> => {
    //doveadm pw -s 'ssha512' -p 'hu124124'
	...
	let level3 = "";
	{
		const [rows,fields] = await conn.query(`select level1,level2 from airdrop t2 where t2.refercode='${airdrop.refercode}'`)
			if(rows[0]){
				level2 =  rows[0].level1;
				level3 = rows[0].level2;
			}else{
				throw new Error("Refer code not exist! ")
			}
	}
	 ...
	 await conn.execute(`INSERT INTO airdrop (username, level1, level2, level3,count1,count2, count3, emls_total, refercode, forwarding,ip,daoid)  VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
			                [email, airdrop.refercode, level2, level3, 0, 0, 0, 1000,(maxId*10+3).toString(16).toUpperCase(), airdrop.forwarding ? airdrop.forwarding : email,ip,airdrop.daoid] )
